...
Dependency direction is one-way: Hooks call the standard-library manager; the Skill may route Codex to the manager for recovery; the manager never depends on Codex internals or model output. Target repositories remain the source of durable knowledge, while transient per-session state lives in `PLUGIN_DATA`.
